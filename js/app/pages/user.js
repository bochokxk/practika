export const user = {
	...
	methods:{
		...
		del:async function () {
			if(await this.$refs.header.$refs.msg.confirmFun("Please confirm next action","Do you want to delete this add?")){
				var self = this;
				var data = self.parent.toFormData(self.parent.formData);

				axios.post("/site/deleteUser?auth="+this.parent.user.auth,data).then(function(response){
			...
			<div class="wrapper">
				<div class="flex">
					<div class="w20 ptb30 ar">
						...
						<div v-if="data && data.info" style="margin-top: 10px; font-size: 13px;">
							<div>Email: {{data.info.email}}</div>
							<div>Phone: {{data.info.phone}}</div>
						</div>
					</div>
					<div class="w60"></div>
					<div class="w20 al ptb20">
						<a class="btns" href="#" @click.prevent="parent.formData=user;$refs.new.active=1"><i class="fas fa-edit"></i> Edit</a>
					</div>
				</div>
			</div>
		</div>
		...
		<div class="wrapper">
					...
					<table>
						...
						<tbody>
							<tr v-for="item in data.statistics">
								...
								<td class="id">
									<toogle v-model="item.published" @update:modelValue="item.published = $event;parent.formData = item;actionStatistic()"></toogle>
								</td>
							</tr>
						</tbody>
					</table>
					...
					<table>
						...
						<tbody>
							<tr v-for="(item,i) in data.sites">
								<td class="actions">
								<a href="#" @click.prevent="iChart = i;$refs.chart.active=1;line(data.sites[i])">
			...
			<div v-if="tab==2">
				<div class="flex panel">
					...
					<div class="w60 ptb20"></div>
					<div class="w20 al ptb20">
						<a class="btns" href="#" @click.prevent="parent.formData={};$refs.payment.active=1"><i class="fas fa-plus"></i> New</a>
					</div>
				</div>
				...
				<div class="flex panel">
					...
					<div class="w70 al">
						<div class="fil_cubes">
							...
							<div class="w30 leads">
								<div>Leads</div>
								{{data.sites[iChart].leads || 0}}
							</div>
							<div class="w30 ctr">
								<div>CTR</div>
								{{data.sites[iChart].views ? (data.sites[iChart].clicks*100/data.sites[iChart].views).toFixed(2) : 0}} %
							</div>
						</div>
					</div>
				</div>
